/**
 * ML Import Pro Worker Logic
 *
 * Extracted from app/api/ml/import-pro/run/route.ts
 * Handles the ML publications scan+multiget import loop with
 * scroll cursor management, rate-limit handling, and progress tracking.
 */
...
export interface ProImportInput {
  account_id: string
  max_seconds?: number
  detail_batch?: number
  concurrency?: number
...
export interface ProImportResult {
  ok: boolean
...
/**
 * Validates the import request preconditions and returns the progress row
 * or an error object suitable for returning as a JSON response.
 */
...
> {
  ...
  if (accountError) {
    return { ok: false, status: 503, body: { error: "Database error", details: accountError.message } }
  }
  if (!account) {
  ...
  if (progressError || !progress) {
    return { ok: false, status: 404, body: { error: "Progress not found. Initialize the import first." } }
  }
...
/**
 * Runs the ML Import Pro scan+multiget loop.
 * This is the core business logic extracted from the route handler.
 */
export async function runProImport(
  supabase: SupabaseClient,
  account: any,
  progress: any,
  input: ProImportInput,
): Promise<ProImportResult> {
  const startTime = Date.now()
  const accountId = input.account_id
  const maxSeconds = input.max_seconds ?? 12
  const batchSize = Math.min(ML_MULTIGET_MAX_IDS, Math.max(1, input.detail_batch ?? ML_MULTIGET_MAX_IDS))
...
/**
 * Handles error cleanup: marks progress as error and fails the process run.
 */
export async function handleImportError(
  supabase: SupabaseClient,
  accountId: string,
  error: any,
  run: Awaited<ReturnType<typeof startRun>> | null,
): Promise<void> {
  try {
    await supabase
      .from("ml_import_progress")
