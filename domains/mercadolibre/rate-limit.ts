 ...
 */
export async function acquireMlToken(account_id: string, cost: number = 1): Promise<RateLimitResult> {
  ...
  try {
    ...
    if (!rateLimitData) {
      const { error: createError } = await supabase.from("ml_rate_limits").insert({
        account_id,
        window_start: now,
        tokens_used: cost,
    ...
    if (newTokensUsed > rateLimitData.tokens_limit) {
      ...
      console.warn(
        `[RATE-LIMIT] Limit exceeded for account ${account_id}: ${newTokensUsed}/${rateLimitData.tokens_limit}`,
      )
