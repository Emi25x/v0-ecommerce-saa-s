/**
 * ML Job handlers — extracted from app/api/ml/jobs/run/route.ts
 *
 * Contains all business logic for ML background jobs:
 * - catalog_index
 * - catalog_optin
 * - buybox_sync
 * - import_single_item
...
const ML_ATTRIBUTES =
  "id,title,price,available_quantity,sold_quantity,status,permalink,thumbnail,listing_type_id,attributes,seller_custom_field"

// Backoff por número de intento: 30s, 2m, 5m, 10m, 30m
export const BACKOFF_SECONDS = [30, 120, 300, 600, 1800]
...
async function executeCatalogOptIn(job: any, supabase: any): Promise<Record<string, unknown>> {
  ...
  if (catalogJobId) {
    ...
    await supabase.rpc("increment_catalog_job_success", { _job_id: catalogJobId }).catch(() => {})
  }
...
export async function executeJob(job: any, supabase: any, instanceId: string) {
  switch (job.type) {
