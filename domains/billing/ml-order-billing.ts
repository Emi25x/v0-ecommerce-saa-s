...
export async function getMLOrderBilling(
  ...
  if (cached) {
    ...
    const needsRevalidation =
      (!!cached.nombre && !cached.doc_numero) || (!!cached.billing_info_missing && ageMs > 2 * 60 * 60 * 1000)
  ...
  const { data: mlAccount } = await adminClient.from("ml_accounts").select("access_token").eq("id", account_id).single()
  ...
  const buyerFallback = [buyer.first_name, buyer.last_name].filter(Boolean).join(" ").trim() || buyer.nickname || null
  ...
  const buyerDirectId = buyer?.identification || buyer?.billing_info?.identification || null
  const buyerDirectDoc: string | null = buyer?.billing_info?.doc_number || buyer?.doc_number || null
  const buyerDirectDocType: string | null = buyer?.billing_info?.doc_type || buyer?.doc_type || null
  ...
  function extractBillingFields(raw: any): {
    ...
    if (isBusiness) {
      ...
      resNombre = [bi.first_name || bi.name, bi.last_name].filter(Boolean).join(" ").trim() || bi.full_name || null
    }
    ...
    let resDocNumero: string | null =
      bi.identification?.number != null
        ? String(bi.identification.number)
        : bi.doc_number
          ? String(bi.doc_number)
          : null
    ...
    if (bi.address) {
      ...
      if (typeof a === "string") {
        ...
        const parts = [a.street_name, a.street_number, a.apartment, cityName, stateName, a.zip_code].filter(Boolean)
  ...
  try {
    ...
    if (billingInfoId) {
      try {
        rawA = await mlFetch(`https://api.mercadolibre.com/orders/billing-info/MLA/${billingInfoId}`)
      } catch {
        /* fallback */
      }
    }
    ...
    try {
      rawB_v2 = await mlFetch(`https://api.mercadolibre.com/orders/${order_id}/billing_info`, { "x-version": "2" })
    } catch {
      /* fallback a V1 */
    }
    ...
    try {
      rawB_v1 = await mlFetch(`https://api.mercadolibre.com/orders/${order_id}/billing_info`)
    } catch {
      /* ignorar */
    }
