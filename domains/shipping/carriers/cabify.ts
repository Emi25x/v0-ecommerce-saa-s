...
export class CabifyLogisticsClient {
  ...
  private async getBearerToken(): Promise<string> {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(`Cabify OAuth error ${res.status}: ${data?.error_description ?? data?.error ?? res.statusText}`)
      }
  ...
  async getParcels(states: CabifyParcelState[], page = 1): Promise<CabifyPaginatedParcels> {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}))
      if (!res.ok)
        throw new Error(`Cabify Logistics API error ${res.status}: ${(data as any)?.message ?? res.statusText}`)
  ...
  async getParcel(parcelId: string): Promise<CabifyParcel> {
    return this.request<CabifyParcel>(
      "GET",
      `${CabifyLogisticsClient.BASE_PATHS.parcels}/${encodeURIComponent(parcelId)}`,
    )
  }
  ...
  async notifyParcel(parcelId: string, event: "ready_to_pickup"): Promise<void> {
    return this.request("POST", `${CabifyLogisticsClient.BASE_PATHS.parcels}/${encodeURIComponent(parcelId)}/notify`, {
      event,
    })
  }
  ...
  async subscribeWebhook(
    hook: "parcel" | "parcelLocation" | "proofCodeGenerated",
    callbackUrl: string,
  ...
  async healthCheck(): Promise<{ ok: boolean; message: string; debug?: unknown }> {
    ...
    try {
      ...
      if (err.name === "AbortError")
        return { ok: false, message: "Cabify Logistics: timeout — sin respuesta del servidor" }
      return { ok: false, message: err.message }
    }
  }
}
...
export function createCabifyClient(config: CabifyConfig, credentials: CabifyCredentials): CabifyLogisticsClient {
  if (!credentials?.client_id || !credentials?.client_secret) {
    throw new Error(
      "Cabify Logistics: client_id y client_secret son requeridos — generarlos en Cabify Logistics → Configuración → API",
    )
  }
