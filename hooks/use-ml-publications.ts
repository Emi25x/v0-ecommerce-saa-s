...
export function useMlPublications() {
  ...
  const loadDuplicates = async () => {
    if (accountId === "all") {
      toast({ title: "Selecciona una cuenta", description: "Elegi una cuenta para buscar duplicados.", variant: "destructive" })
      return
    }
  ...
  const closePub = async (pub: Publication) => {
    if (!confirm(`\u00bfEliminar ${pub.ml_item_id} en MercadoLibre?\n\nEsto cierra la publicacion en ML. Para que desaparezca del panel, despues podes reimportar o sincronizar.`)) return
    ...
    try {
      ...
      const data = await res.json()
      if (data.ok) {
        toast({ title: "Publicacion eliminada en ML", description: `${pub.ml_item_id} cerrada en MercadoLibre. Recargando duplicados...` })
        loadDuplicates()
        loadCounts(accountId)
      } else {
  ...
  const syncWeights = async () => {
    if (accountId === "all") {
      toast({ title: "Selecciona una cuenta", description: "Elegi una cuenta antes de sincronizar pesos.", variant: "destructive" })
      return
    }
    ...
    try {
      ...
      if (data.ok) {
        ...
        toast({ title: "Sincronizacion completada", description: `${data.updated} pesos actualizados, ${data.missing} sin peso en ML.` })
        load(page)
      } else {
  ...
  const backfillSku = async () => {
    if (accountId === "all") {
      toast({ title: "Selecciona una cuenta", description: "Elegi una cuenta antes de hacer backfill.", variant: "destructive" })
      return
    }
  ...
  const syncWithML = async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        toast({ title: "Rate limit ML", description: `Espera ${data.wait_seconds ?? 60}s y reintenta.`, variant: "destructive" })
      } else {
        toast({ title: "Error al sincronizar", description: data.error ?? "Error desconocido", variant: "destructive" })
      }
    } catch (err: any) {
