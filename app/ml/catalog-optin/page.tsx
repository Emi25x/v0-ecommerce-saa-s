...
export default function CatalogOptinPage() {
  ...
  const loadPubs = useCallback(async () => {
    ...
    setLoading(true)
    setPubs([])
    addLog("Cargando publicaciones elegibles para optin...")
    try {
      ...
      addLog(`${enriched.length} publicaciones cargadas de ${data.total} elegibles`, "ok")
    } finally {
      setLoading(false)
    }
  }, [accountId, addLog])

  useEffect(() => { if (accountId) loadPubs() }, [accountId, loadPubs])

  // ── Resolve catalog_product_id ─────────────────────────────────────────────

  const resolveAll = useCallback(async () => {
    if (!accountId || running) return
    setRunning(true)
    abortRef.current = false
    const toResolve = pubs.filter(p => p.resolve_status === "pending")
    addLog(`Resolviendo ${toResolve.length} EANs contra catalogo ML...`)
    ...
    for (const pub of toResolve) {
      ...
      if (data?.status === "resolved") {
        ...
        setPubs(prev => prev.map(p => p.id === pub.id ? {
          ...
          ambiguous_options: data.results,
        } : p))
        ambiguous++
    ...
    addLog(`Resolucion completa: ${ok} resueltos | ${notFound} sin match | ${ambiguous} ambiguos | ${errors} errores`, ok > 0 ? "ok" : "warn")
    setRunning(false)
  }, [accountId, pubs, running, addLog])
  ...
  const runOptin = useCallback(async () => {
    ...
    const toOptin = pubs.filter(p => p.resolve_status === "resolved" && p.catalog_product_id && p.optin_status !== "ok")
    addLog(`Iniciando optin ${dryRun ? "DRY RUN" : "LIVE"} sobre ${toOptin.length} items...`)
    let ok = 0, failed = 0
    ...
    setRunning(false)
  }, [accountId, pubs, dryRun, confirmLive, running, addLog])
  ...
  return (
    <main className="flex flex-col gap-6 p-6 max-w-6xl mx-auto font-sans">
      ...
      {/* Progress bar — solo durante resolucion */}
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Button
          ...
          size="sm"
        >
          {running ? "Resolviendo..." : `Resolver EANs (${counts.pending} pendientes)`}
        </Button>
        ...
        <div className="flex items-center gap-3 border border-border rounded-lg px-3 py-2">
          ...
          <Button
            onClick={runOptin}
            disabled={running || loading || counts.resolved === 0 || (!dryRun && !confirmLive)}
            size="sm"
            className={dryRun ? "" : "bg-orange-600 hover:bg-orange-700 text-white"}
          >
            {dryRun ? `Simular optin (${counts.resolved})` : `Optin LIVE (${counts.resolved})`}
          </Button>
        </div>
        ...
        <div className="rounded-lg border border-border overflow-x-auto">
            ...
            <tbody>
              {filteredPubs.slice(0, 500).map((p) => (
                <tr key={p.id} className="border-b border-border/50 hover:bg-muted/20">
                  ...
                  <td className="px-3 py-1.5 text-foreground max-w-[260px] truncate">{p.title}</td>
          ...
          {filteredPubs.length > 500 && (
            <div className="px-3 py-2 text-xs text-muted-foreground border-t border-border">
              Mostrando 500 de {filteredPubs.length} — usá el filtro para ver segmentos especificos
            </div>
          )}
        </div>
      )}
...
function AmbiguousSelect({ options, onSelect }: { options: { id: string; name: string }[]; onSelect: (id: string) => void }) {
  return (
    <select
      className="h-7 rounded border border-yellow-400/40 bg-background text-xs text-yellow-300 px-2"
      defaultValue=""
