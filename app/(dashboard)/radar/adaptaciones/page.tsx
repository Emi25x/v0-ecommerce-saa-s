  ...
  return (
    <div className="p-6 space-y-5 max-w-6xl mx-auto">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Sugerencias de adaptación editorial detectadas por el Radar
          </p>
        </div>
            ...
            <tbody className="divide-y divide-border">
              {rows.map((row) => (
                <tr key={row.id} className="hover:bg-muted/20 cursor-pointer" onClick={() => setSelected(row)}>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${PRIORITY_COLOR[row.priority] ?? "bg-muted"}`}
                    >
                      {row.priority}
                    </span>
                  </td>
          ...
          <DialogContent className="max-w-md">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex gap-2">
                <span
                  className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${PRIORITY_COLOR[selected.priority] ?? "bg-muted"}`}
                >
                  {selected.priority}
                </span>
                <span
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  ...
                  {TYPE_LABEL[selected.adaptation_type] ?? selected.adaptation_type}
                </div>
                ...
                {selected.due_date && (
                  <div>
                    ...
                    {new Date(selected.due_date).toLocaleDateString("es-AR")}
                  </div>
                )}
              </div>
            </div>
      ...
      <Dialog open={showNew} onOpenChange={setShowNew}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Nueva adaptación</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Select
                  value={form.adaptation_type}
                  onValueChange={(v) => setForm((f) => ({ ...f, adaptation_type: v }))}
                >
            ...
            <div className="space-y-1">
              ...
              <Input
                className="h-8 text-sm"
                placeholder="+20% ventas, nuevo segmento…"
