...
export default function HuecosPage() {
  ...
  const [form, setForm] = useState({
    ...
    demand_score: "50", supply_score: "20", example_isbns: "",
  })
  ...
  const handleCreate = async () => {
    ...
    setSaving(true)
    try {
      const isbns = form.example_isbns.split(",").map(s => s.trim()).filter(Boolean)
  ...
  return (
    <div className="p-6 space-y-5 max-w-6xl mx-auto">
      ...
      {selected && (
        <Dialog open onOpenChange={() => setSelected(null)}>
          <DialogContent className="max-w-md">
            <DialogHeader><DialogTitle>{selected.category}</DialogTitle></DialogHeader>
            <div className="space-y-3 text-sm">
              ...
              <div className="grid grid-cols-3 gap-3 text-center">
                {[
                  ...
                  { label: "Gap",     value: Number(selected.gap_score ?? 0).toFixed(1), color: "text-amber-400 font-bold" },
                ].map(m => (
                ...
                <div>
                  ...
                  <div className="flex gap-1.5 flex-wrap">
                    {selected.example_isbns.map(i => (
                      <span key={i} className="text-[10px] bg-muted px-2 py-0.5 rounded-full font-mono">{i}</span>
                    ))}
                  </div>
                </div>
              )}
      ...
      <Dialog open={showNew} onOpenChange={setShowNew}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>Nuevo hueco de mercado</DialogTitle></DialogHeader>
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              ...
              <Input className="h-8 text-sm" value={form.sub_category} onChange={e => setForm(f => ({ ...f, sub_category: e.target.value }))} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label className="text-xs">Score demanda (0-100)</Label>
                <Input className="h-8 text-sm" type="number" min={0} max={100} value={form.demand_score} onChange={e => setForm(f => ({ ...f, demand_score: e.target.value }))} />
              </div>
              <div className="space-y-1">
                <Label className="text-xs">Score oferta (0-100)</Label>
                <Input className="h-8 text-sm" type="number" min={0} max={100} value={form.supply_score} onChange={e => setForm(f => ({ ...f, supply_score: e.target.value }))} />
              </div>
            </div>
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              ...
              <Input className="h-8 text-sm font-mono" value={form.example_isbns} onChange={e => setForm(f => ({ ...f, example_isbns: e.target.value }))} />
            </div>
          </div>
          <DialogFooter>
