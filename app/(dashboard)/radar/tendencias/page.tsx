  ...
  if (delta > 0)
    return (
      <span className="inline-flex items-center gap-0.5 text-xs font-semibold text-emerald-400">
        ...
        {pct !== null && <span className="text-[10px] text-emerald-500/80">({fmtPct(pct)})</span>}
      </span>
    )
  if (delta < 0)
    return (
...
export default function TendenciasPage() {
  ...
  const loadSignals = useCallback(async () => {
    ...
    try {
      ...
      if (d.ok) {
        setSignals(d.rows)
        setSigTotal(d.total ?? d.rows.length)
      }
    } finally {
      setLoadingSig(false)
    }
  ...
  const renderVentasTable = () => {
    ...
    if (errorV) return <div className="p-6 text-center text-sm text-rose-400">{errorV}</div>
          ...
          <tbody className="divide-y divide-border/60">
            {ventasItems.map((item, i) => (
              <tr key={i} className="hover:bg-muted/20 transition-colors">
                ...
                {isVendedores && (
                  <td className="px-3 py-2 text-xs text-muted-foreground truncate max-w-[120px]">{item.seller_name}</td>
                )}
                <td className="px-3 py-2 text-right font-mono font-semibold">{item.current_qty}</td>
                <td className="px-3 py-2 text-right font-mono text-muted-foreground">{item.prev_qty ?? "—"}</td>
      ...
      <div>
        <div className="flex items-center justify-between gap-4 mb-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">Libros con ventas en alza por período y fuente</p>
          </div>
        ...
        <Card className="overflow-hidden">
          ...
          <div className="flex flex-wrap items-center gap-3 px-4 py-3 border-b border-border bg-muted/10">
              ...
              {PERIOD_OPTS.map((opt) => (
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-medium transition-colors ${
                    days === opt.days ? "bg-primary text-primary-foreground" : "hover:bg-muted/50 text-muted-foreground"
                  }`}
                >
                  {opt.label}
                </button>
            ...
            {ventasTotal > 0 && <span className="ml-auto text-xs text-muted-foreground">{ventasTotal} libros</span>}
          </div>
          ...
          {source === "vendedores" && showAddSeller && (
            <div className="px-4 py-3 border-b border-border bg-muted/5 space-y-3">
              {sellers.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {sellers.map((s) => (
                    <span
                      key={s.seller_id}
                      className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-muted/30 border border-border text-xs"
                    >
                      {s.nickname}
                      <span className="text-muted-foreground font-mono text-[10px]">#{s.seller_id}</span>
                      <button
                        onClick={() => removeSeller(s.seller_id)}
                        className="text-muted-foreground hover:text-rose-400 ml-0.5"
                      >
                        <X className="h-3 w-3" />
                      </button>
                    </span>
              ...
              <p className="text-[10px] text-muted-foreground">
                El Seller ID se obtiene del perfil del vendedor en ML (número en la URL). Si no ingresás nombre, se
              ...
              <tbody className="divide-y divide-border">
                {signals.map((s) => (
                  <tr key={s.id} className="hover:bg-muted/20">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${SIGNAL_COLOR[s.signal_type] ?? "bg-muted"}`}
                      >
                        {s.signal_type}
                      </span>
                    </td>
            ...
            {Object.entries(byType).map(([type, sigs]) => (
              <div key={type}>
                <div className="flex items-center gap-2 mb-3">
                  <span
                    className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${SIGNAL_COLOR[type] ?? "bg-muted"}`}
                  >
                    {type}
                  </span>
