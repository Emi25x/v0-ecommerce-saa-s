    ...
    <div className="p-6 space-y-5 max-w-6xl mx-auto">
      ...
      {fetchResult && (
        <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 px-4 py-3 flex items-center gap-3 text-sm">
          ...
          <span>
            ...
            <span className="font-semibold text-emerald-400">{fetchResult.total_adaptations}</span> adaptaciones
            detectadas
          </span>
          <button
      ...
      {loading ? (
        ...
        <Card className="p-10 text-center space-y-3">
          ...
          <p className="text-muted-foreground text-sm">Sin adaptaciones detectadas aún.</p>
            ...
            <tbody className="divide-y divide-border">
              {rows.map((row) => (
                <tr key={row.id} className="hover:bg-muted/20 cursor-pointer" onClick={() => setSelected(row)}>
                  ...
                  <td className="px-4 py-3 text-xs text-muted-foreground">{row.detected_author ?? "—"}</td>
                  <td className="px-4 py-3">
                    <span
                      className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${SOURCE_COLOR[row.source] ?? "bg-muted text-muted-foreground border-border"}`}
                    >
                      {row.source}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${PROJECT_TYPE_COLOR[row.project_type]}`}
                    >
                  ...
                  <td className="px-4 py-3">
                    {row.project_status ? (
                      <span
                        className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${STATUS_COLOR[row.project_status] ?? "bg-muted text-muted-foreground"}`}
                      >
      ...
      {selected && (
        <Dialog open onOpenChange={() => setSelected(null)}>
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle className="leading-snug">{selected.detected_book ?? selected.title}</DialogTitle>
            </DialogHeader>
            <div className="space-y-4 text-sm">
              <div className="flex flex-wrap gap-2">
                <span
                  className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${SOURCE_COLOR[selected.source] ?? "bg-muted border-border"}`}
                >
                  {selected.source}
                </span>
                <span
                  className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${PROJECT_TYPE_COLOR[selected.project_type]}`}
                >
                  {PROJECT_TYPE_LABEL[selected.project_type]}
                </span>
                {selected.project_status && (
                  <span
                    className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${STATUS_COLOR[selected.project_status] ?? "bg-muted"}`}
                  >
                    {STATUS_LABEL[selected.project_status] ?? selected.project_status}
                  </span>
                )}
                <span
                  className={`text-[10px] font-semibold px-2 py-0.5 rounded-full bg-muted ${scoreColor(selected.confidence_score)}`}
                >
              ...
              {selected.detected_book && (
                <div className="grid grid-cols-2 gap-2 rounded-md border border-border bg-muted/20 p-3 text-xs">
                  <div>
                    <span className="text-muted-foreground">Libro detectado: </span>
                    <span className="font-medium">{selected.detected_book}</span>
                  </div>
                  {selected.detected_author && (
                    <div>
                      <span className="text-muted-foreground">Autor: </span>
                      <span className="font-medium">{selected.detected_author}</span>
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  ...
                  {relDate(selected.published_at)}
                </div>
                <div>
                  <span className="text-muted-foreground">Detectado: </span>
                  {relDate(selected.created_at)}
                </div>
                <div>
                  <span className="text-muted-foreground">Tiempo est. estreno: </span>12-24 meses
                </div>
                {selected.opportunity_id && (
                  <div>
                    <span className="text-muted-foreground">Oportunidad: </span>
                    <a href="/radar/oportunidades" className="text-blue-400 hover:text-blue-300">
                      generada →
                    </a>
                  </div>
                )}
