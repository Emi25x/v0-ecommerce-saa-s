...
import {
  ...
  Clock, Truck, UserSearch, X, FileText,
} from "lucide-react"
...
interface BillingCache { state: BillingLoadState; data?: BillingInfo; error?: string }
          ...
          <div>
            ...
            {order.factura ? (
              <div className="rounded-md border border-border bg-muted/20 p-3 text-sm space-y-1">
                ...
                {order.factura.cae && (
                  <p className="text-xs text-muted-foreground">CAE: <span className="font-mono">{order.factura.cae}</span></p>
                )}
              </div>
            ) : (
                  ...
                  {items.map((it, i) => (
                    <div key={i} className="flex items-start justify-between gap-3 text-sm">
                      ...
                      <div className="text-right flex-shrink-0">
                        <p className="text-xs text-muted-foreground">{it.quantity} × {fmt(it.unit_price, order.currency_id)}</p>
...
export default function MLOrdersPage() {
  ...
  useEffect(() => { setPage(0); load(0) }, [accountId, statusFilter])   // eslint-disable-line
  useEffect(() => { if (page > 0) load(page) }, [page])                 // eslint-disable-line
  ...
  const fetchBilling = useCallback(async (order: MLOrder) => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        if (data.billing_info_missing) {
          toast({ title: "Sin datos fiscales en ML", description: "Se usará Consumidor Final al facturar.", variant: "destructive" })
        } else {
      ...
      setFetchingBilling(prev => { const s = new Set(prev); s.delete(key); return s })
    }
  }, [toast])
          ...
          <div className="rounded-lg border border-border overflow-hidden">
                              ...
                              <div className="flex items-center justify-center gap-1">
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    ...
                                    >
                                      <UserSearch className={`h-3.5 w-3.5 transition-colors ${
                                        billing?.state === "loaded"   ? "text-green-400"
                                        : billing?.state === "error"  ? "text-red-400"
                                        : isFetchingBilling           ? "text-blue-400 animate-pulse"
                                        : "text-muted-foreground hover:text-foreground"
                                      }`} />
                                    </button>
                                  </TooltipTrigger>
                                  <TooltipContent side="top">
                                    {billing?.state === "loaded"  ? `${billing.data?.nombre ?? "Datos cargados"}` :
                                     billing?.state === "error"   ? "Error — reintentar"           :
                                     isFetchingBilling            ? "Obteniendo..."                :
                                     "Obtener datos fiscales"}
                                  </TooltipContent>
                                </Tooltip>
            ...
            <div className="flex items-center justify-between px-4 py-2.5 border-t border-border bg-muted/10">
              ...
              <div className="flex items-center gap-1.5">
                <Button
                  size="sm" variant="outline" className="h-7 px-2.5"
