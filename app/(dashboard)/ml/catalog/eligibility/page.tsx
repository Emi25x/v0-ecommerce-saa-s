...
export default function CatalogEligibilityPage() {
  ...
  const handleOptIn = async (row: Row) => {
    ...
    try {
      ...
      if (data.ok || data.status === "already_in_catalog") {
        ...
        toast({
          title: "Error en opt-in",
          description: data.error ?? data.status ?? "Error desconocido",
          variant: "destructive",
        })
      }
    } catch (e: any) {
  ...
  const handleRun = useCallback(
    async (force = false) => {
      ...
      addLog(`Iniciando indexacion (${accountId}${force ? ", forzando re-proceso" : ""})`, "info")
      ...
      try {
        while (!abortRef.current) {
          ...
          if (data.done || !data.has_more) {
            addLog(`Completado. Total procesadas: ${totalProcessed}, con match: ${totalMatched}`, "ok")
            break
          }
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground max-w-xl">
              Publicaciones con ISBN / EAN consultadas contra la API de productos de Mercado Libre. Las que tienen match
              único se marcan elegibles para opt-in al catálogo.
            </p>
          </div>
        ...
        <div className="flex flex-wrap gap-3 items-end justify-between">
          ...
          <div className="flex gap-2 items-center">
            ...
            <Tooltip>
              ...
              <TooltipContent>Hace opt-in al catálogo de todas las publicaciones elegibles de la cuenta</TooltipContent>
            </Tooltip>
          ...
          <div className="rounded-xl border border-border overflow-hidden">
            {running && runProgress && (
              <div className="flex items-center gap-4 px-4 py-2.5 bg-muted/20 border-b border-border text-xs text-muted-foreground">
                ...
                <span>
                  Procesadas: <span className="text-foreground font-medium tabular-nums">{runProgress.processed}</span>
                  &nbsp;·&nbsp; Con match:{" "}
                  <span className="text-green-400 font-medium tabular-nums">{runProgress.matched}</span>
                </span>
              </div>
            )}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/30">
                    <th className="text-left px-4 py-3 font-medium text-muted-foreground whitespace-nowrap">Item</th>
                          ...
                          <tr key={row.id} className="border-b hover:bg-muted/20 transition-colors group">
                            ...
                            <td className="px-4 py-3">
                              {row.catalog_product_id ? (
                                <div className="flex items-center gap-1.5">
                                  <span className="font-mono text-xs text-blue-400">{row.catalog_product_id}</span>
                                  <button
