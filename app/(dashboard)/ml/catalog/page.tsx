...
export default function MLCatalogMigrationPage() {
  ...
  const runLoop = useCallback(
    async (runUrl: string, body: any, label: string) => {
      setRunning(true)
      ...
      try {
        while (!abortRef.current) {
          ...
          if (data.error) {
            addLog(`Error en ${label}: ${data.error}`)
            break
          }
          ...
          if (data.cumulative_processed !== undefined)
            parts.push(`total=${data.cumulative_processed}/${data.total_estimated || "?"}`)
  ...
  const handleResolve = useCallback(async () => {
    ...
    if (startData.pending === 0) {
      addLog(startData.message || "Sin candidatos pendientes")
      return
    }
  ...
  const handleMigrate = useCallback(
    async (dryRun: boolean) => {
      ...
      if (startData.pending === 0) {
        addLog(startData.message || "Sin items listos para optin")
        return
      }
  ...
  const progressPct =
    job && job.total_estimated > 0 ? Math.min(100, Math.round((job.processed_count / job.total_estimated) * 100)) : 0

  const phaseLabel =
    job?.phase === "audit"
      ? "Auditoria"
      : job?.phase === "resolve_catalog_product"
        ? "Resolucion"
        : job?.phase === "migrate"
          ? "Optin"
          : "-"
    ...
    <main className="flex flex-col gap-6 p-6 max-w-5xl mx-auto font-sans">
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-muted-foreground">
          Aplica OPTIN a publicaciones tradicionales existentes. No crea publicaciones nuevas. Soporta 50 000+ items con
          resume automatico.
        </p>
      </div>
        ...
        <div className="rounded-lg border border-border bg-card p-5 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            <div className="flex justify-between text-xs text-muted-foreground">
              ...
              <span>
                {job.total_estimated > 0 ? `de ${job.total_estimated.toLocaleString()} estimadas` : "total desconocido"}
              </span>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              {
                label: "Ya catalogo",
                val: job.already_catalog_count,
                color: "text-green-400",
                tip: "Ya eran catalogo antes de empezar",
              },
              {
                label: "Sin EAN",
                val: job.no_ean_count,
                color: "text-zinc-500",
                tip: "No tienen EAN — no se pueden resolver",
              },
              {
                label: "Migrables",
                val: counts.migrables,
                color: "text-blue-400",
                tip: "Resueltos con match unico — listos para optin",
              },
              {
                label: "Sin match",
                val: counts.no_match,
                color: "text-orange-400",
                tip: "not_found o ambiguos — no entran al job",
              },
            ].map(({ label, val, color, tip }) => (
              <div
                key={label}
                title={tip}
                className="flex flex-col items-center rounded-md border border-border bg-background p-3 cursor-help"
              >
