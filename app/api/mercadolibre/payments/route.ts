...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const account of accounts) {
      ...
      try {
        const maxPages = 10 // Aumentado a 10 páginas para obtener hasta 500 órdenes
        ...
        for (let page = 0; page < maxPages; page++) {
            ...
            for (const order of mlData.results) {
              ...
              if (order.payments && Array.isArray(order.payments)) {
                ...
                for (const payment of order.payments) {
                  ...
                  const marketplaceFee = totalSaleFee / order.payments.length // Distribute fee among payments
          ...
          // If we got less than ordersPerPage results, no need to fetch more pages
