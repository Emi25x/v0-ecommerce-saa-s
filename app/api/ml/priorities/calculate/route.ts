...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!products?.length) return NextResponse.json({ ok: true, processed: 0 })
    ...
    for (const product of products) {
      ...
      // ── Demand score (0-30) ────────────────────────────────────────────
      // Proxy: if there are active listings it signals the market exists
      // We can't know sales without order data so we use listing count as proxy
      const competitorListings = mlFiltered.length
      let scoreDemand = 0
      if (competitorListings === 0)  scoreDemand = 20  // blue ocean
      else if (competitorListings <= 3) scoreDemand = 28
      else if (competitorListings <= 8) scoreDemand = 22
      else scoreDemand = 12

      // ── Competition score (0-20) — less competition = higher score ─────
      ...
      else scoreStock = 0  // no stock = can't publish
      ...
      if (cost > 0 && price > 0) {
        ...
        scoreProfitability = 7  // unknown cost, has a price
      }
