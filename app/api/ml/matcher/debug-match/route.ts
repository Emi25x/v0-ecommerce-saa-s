...
export async function GET(request: NextRequest) {
  ...
  const { data: allProducts } = await sessionClient.from("products").select("id, ean, gtin, isbn, sku")
  ...
  for (const p of allProducts ?? []) {
    for (const raw of [p.ean, p.gtin]) {
      if (raw) {
        const k = norm(raw)
        if (k) addToIndex(eanIndex, k, p.id)
      }
    }
    if (p.isbn) {
      const k = norm(p.isbn)
      if (k) addToIndex(isbnIndex, k, p.id)
    }
    if (p.sku) {
      const k = norm(p.sku)
      if (k) addToIndex(skuIndex, k, p.id)
    }
  }
  ...
  const pubDebug = (pubs ?? []).map((pub) => {
    ...
    if (pub.gtin) {
      ...
      if (k) {
        if (!eans.includes(k)) eans.push(k)
      }
    }
    if (pub.isbn) {
    ...
    outer: for (const [ids, index, type] of [
      ...
      for (const key of ids) {
        ...
        if (pids.length === 1) {
          matchResult = { found: true, type, key, product_id: pids[0] }
          break outer
        }
        if (pids.length > 1) {
          matchResult = { found: false, type, key, reason: "ambiguous", count: pids.length }
          break outer
        }
      }
    }
