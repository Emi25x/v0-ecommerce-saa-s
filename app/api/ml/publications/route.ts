...
export async function GET(req: NextRequest) {
  try {
    ...
    const alertsMode = searchParams.get("alerts_mode") as "eligible_catalog" | "under_review" | "about_to_pause" | null
      ...
      const [
        ...
        head((qb) =>
          qb
            ...
            .or("catalog_listing.is.null,catalog_listing.eq.false")
            .is("catalog_linked_item_id", null),
        ),
      ])
    ...
    if (accountId) {
      const { data: prog } = await supabase
        ...
        .select(
          "status, publications_total, ml_items_seen_count, db_rows_upserted_count, upsert_errors_count, last_run_at, last_sync_batch_at, finished_at",
        )
