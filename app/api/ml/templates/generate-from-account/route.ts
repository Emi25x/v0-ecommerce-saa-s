...
export async function POST() {
  try {
    ...
    const { data: account, error: accountError } = await supabase.from("ml_accounts").select("*").single()
    ...
    const detailsResponse = await fetch(`https://api.mercadolibre.com/items?ids=${itemIds}`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    })
    ...
    for (const itemWrapper of detailsData) {
      ...
      // Buscar en atributos
      const eanAttr = item.attributes?.find((a: any) => a.id === "GTIN" || a.id === "EAN" || a.id === "ISBN")
    ...
    descriptionTemplate = descriptionTemplate.replace(/(TITULO DEL LIBRO:?\s*)[^\n]+/gi, "$1{title}")
    descriptionTemplate = descriptionTemplate.replace(/(TITULO:?\s*)[^\n]+/gi, "$1{title}")
    descriptionTemplate = descriptionTemplate.replace(/(AUTOR:?\s*)[^\n]+/gi, "$1{author}")
    descriptionTemplate = descriptionTemplate.replace(/(EDITORIAL:?\s*)[^\n]+/gi, "$1{brand}")
    descriptionTemplate = descriptionTemplate.replace(/(ISBN:?\s*)[^\n]+/gi, "$1{ean}")
    descriptionTemplate = descriptionTemplate.replace(/(Fecha de publicaci[oó]n:?\s*)[^\n]+/gi, "$1{year_edition}")

    // Páginas
    descriptionTemplate = descriptionTemplate.replace(/(P[AÁ]GINAS:?\s*)[^\n]+/gi, "$1{pages}")
    descriptionTemplate = descriptionTemplate.replace(/(N[UÚ]MERO DE P[AÁ]GINAS:?\s*)[^\n]+/gi, "$1{pages}")

    // Encuadernación
    descriptionTemplate = descriptionTemplate.replace(/(ENCUADERNACI[OÓ]N:?\s*)[^\n]+/gi, "$1{binding}")
    descriptionTemplate = descriptionTemplate.replace(/(FORMATO:?\s*)[^\n]+/gi, "$1{binding}")
    descriptionTemplate = descriptionTemplate.replace(/(TAPA:?\s*)[^\n]+/gi, "$1{binding}")
    ...
    descriptionTemplate = descriptionTemplate.replace(/(MEDIDAS:?\s*)[^\n]+/gi, "$1{width} x {height} x {thickness}")

    // Categoría
    descriptionTemplate = descriptionTemplate.replace(/(CATEGOR[IÍ]A:?\s*)[^\n]+/gi, "$1{category}")
    descriptionTemplate = descriptionTemplate.replace(/(G[EÉ]NERO:?\s*)[^\n]+/gi, "$1{subject}")
    descriptionTemplate = descriptionTemplate.replace(/(MATERIA:?\s*)[^\n]+/gi, "$1{subject}")
    descriptionTemplate = descriptionTemplate.replace(/(TEM[AÁ]TICA:?\s*)[^\n]+/gi, "$1{subject}")

    // Idioma
    descriptionTemplate = descriptionTemplate.replace(/(IDIOMA:?\s*)[^\n]+/gi, "$1{language}")

    // Reemplazar la reseña/sinopsis específica por variable
    descriptionTemplate = descriptionTemplate.replace(/(RESE[NÑ]A:?\s*)[^]+$/gi, "$1{description}")
    descriptionTemplate = descriptionTemplate.replace(/(SINOPSIS:?\s*)[^]+$/gi, "$1{description}")
