...
export async function GET(request: Request) {
  ...
  try {
    ...
    const { data: account } = await supabase.from("ml_accounts").select("*").single()
    ...
    const itemResponse = await fetch(`https://api.mercadolibre.com/items/${itemId}?include_attributes=all`, {
      headers: { Authorization: `Bearer ${validAccount.access_token}` },
    })
    ...
    const relevantInfo = {
      ...
      attributes: item.attributes?.map(
        (attr: { id: string; name: string; value_id: string | null; value_name: string | null }) => ({
          id: attr.id,
          name: attr.name,
