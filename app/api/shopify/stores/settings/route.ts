...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (!store_id) return NextResponse.json({ ok: false, error: "store_id es requerido" }, { status: 400 })
    ...
    const { error } = await supabase.from("shopify_stores").update(patch).eq("id", store_id)
