...
async function fetchAndStoreSnapshot(
  ...
  try {
    ...
    const param = source_type === "categoria"
...
async function getPropiasTrends(supabase: any, days: number, limit: number, account_id: string | null) {
  ...
  const [curItemsRes, prevItemsRes] = await Promise.all([
    curIds.length  ? supabase.from("order_items").select("title,sku,quantity,total_price").in("order_id", curIds)  : { data: [] },
    prevIds.length ? supabase.from("order_items").select("title,sku,quantity,total_price").in("order_id", prevIds) : { data: [] },
  ])
...
async function getCategoryTrends(supabase: any, days: number, limit: number, category_id: string) {
  ...
  const items = (latest ?? []).map((item: any) => {
...
export async function GET(req: NextRequest) {
  try {
    ...
    switch (source) {
      case "propias":    return getPropiasTrends(supabase, days, limit, account_id)
      case "categoria":  return getCategoryTrends(supabase, days, limit, category_id)
      case "vendedores": return getVendedoresTrends(supabase, days, limit)
      default:
