...
export async function GET(req: NextRequest) {
  ...
  function applyFilters(q: any): any {
    ...
    if (facturado === "si" && facturadaIds.length > 0) q = q.in("ml_order_id", facturadaIds)
    if (facturado === "no" && facturadaIds.length > 0) q = q.not("ml_order_id", "in", `(${facturadaIds.join(",")})`)
    return q
  }
  ...
  let dataQ = supabase
    .from("ml_orders")
    .select(
      "ml_order_id, status, date_created, total_amount, currency_id, buyer_nickname, buyer_id, shipping_status, pack_id, items_json",
    )
...
export async function POST(req: NextRequest) {
  ...
  const { error } = await supabase.from("ml_order_facturas").upsert(rows, { onConflict: "ml_order_id,ml_account_id" })
