...
export async function POST(request: Request) {
  try {
    ...
    if (cert_pem) {
      ...
      payload.certificado_pem = cert_pem
    }
    if (clave_pem) {
      payload.clave_pem = clave_pem
      payload.private_key_pem = clave_pem
    }
    ...
    if (id) {
      ...
      ;({ data, error } = await supabase.from("arca_config").insert(payload).select(SELECT_COLS).single())
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const { data: emp } = await supabase.from("arca_config").select("id").eq("id", id).eq("user_id", user.id).single()

    if (!emp) return NextResponse.json({ error: "Empresa no encontrada" }, { status: 404 })

    // Verificar que no tenga facturas
    const { count } = await supabase.from("facturas").select("id", { count: "exact", head: true }).eq("empresa_id", id)

    if ((count ?? 0) > 0) {
      return NextResponse.json(
        { error: `No se puede eliminar: tiene ${count} factura(s) asociada(s).` },
        { status: 409 },
      )
    }
