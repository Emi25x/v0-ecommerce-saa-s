...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  if (hasCredentials) {
    ...
    const { data: existing } = await supabase.from("carriers").select("credentials").eq("slug", slug).maybeSingle()
  ...
  const { error } = await supabase.from("carriers").update(update).eq("slug", slug)
