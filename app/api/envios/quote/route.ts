...
export async function POST(request: NextRequest) {
  try {
    ...
    const { carrier_slug = "fastmail", origen_cp, destino_cp, peso_g, valor, dimensiones } = body

    if (!origen_cp || !destino_cp || !peso_g) {
      return NextResponse.json({ error: "origen_cp, destino_cp y peso_g son requeridos" }, { status: 400 })
    }
