...
import {
  ...
  FileText, Zap, Filter, Building2, AlertTriangle, ShoppingCart, Download,
} from "lucide-react"
...
function FinancialBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${map[status] ?? "bg-muted/30 text-muted-foreground border-border"}`}>
...
function FulfillmentBadge({ status }: { status: string | null }) {
  if (!status || status === "unfulfilled") {
    return <span className="inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium bg-amber-500/15 text-amber-400 border-amber-500/30">Sin enviar</span>
  }
  const map: Record<string, string> = {
  ...
  return (
    <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium ${map[status] ?? "bg-muted text-muted-foreground border-border"}`}>
      {labels[status] ?? status}
    </span>
  )
}
...
function extractDocFromOrder(order: ShopifyOrder): { docNro: string | null; docTipo: string | null } {
  ...
  let docNro = find("cuit", "nro_doc", "doc_number", "dni", "numero_documento", "identification", "numero_cuit", "numero_dni")
  ...
  const docTipo = find("tipo_doc", "doc_type", "tipo_documento")
    || (docNro && docNro.length === 11 ? "CUIT" : docNro ? "DNI" : null)
...
export default function ShopifyBillingPage() {
  ...
  // ── Cargar órdenes ─────────────────────────────────────────────────────────
  const loadOrders = useCallback(async (pageInfo = "") => {
  ...
  useEffect(() => {
    if (activeStore) { setCursorStack([]); loadOrders() }
  ...
  const processOrders = async (
    ...
    for (const order of selOrders) {
      ...
      try {
        ...
        const condIva = tipoDoc === 80
          ? "responsable_inscripto"
          : tipoDoc === 96 ? "consumidor_final" : "consumidor_final"

        const ba = order.billing_address || {}
        const domicilio = [ba.address1, ba.city, ba.province, ba.zip, ba.country]
  ...
  const handleBuscar = () => { setCursorStack([]); loadOrders() }
  ...
  return (
    <div className="space-y-5 p-1">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <Button variant="outline" size="sm" onClick={() => { setCursorStack([]); loadOrders() }} disabled={loading} className="gap-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          <div className="space-y-1.5">
            ...
            <Select value={filterFinancial} onValueChange={setFilterFinancial}>
              ...
              <SelectContent>
                {FINANCIAL_STATUS_OPTS.map(o => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-1.5">
            ...
            <Select value={filterFulfillment} onValueChange={setFilterFulfillment}>
              <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
              <SelectContent>
                {FULFILLMENT_STATUS_OPTS.map(o => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-1.5">
          ...
          <div className="flex-1 min-w-0">
            ...
            {batchResult.errors.length > 0 && (
              <ul className="mt-2 space-y-0.5">
                {batchResult.errors.map((e, i) => <li key={i} className="text-xs text-red-400">{e}</li>)}
              </ul>
            )}
            {batchResult.warnings.length > 0 && (
              <details className="mt-2">
                <summary className="text-xs text-amber-400 cursor-pointer select-none flex items-center gap-1">
                  ...
                  {batchResult.warnings.length} orden{batchResult.warnings.length !== 1 ? "es" : ""} sin CUIT/DNI (facturadas como Consumidor Final)
                </summary>
                <ul className="mt-1.5 space-y-0.5 pl-4">
                  {batchResult.warnings.map((w, i) => <li key={i} className="text-xs text-muted-foreground">{w}</li>)}
                </ul>
              </details>
            )}
          </div>
      ...
      <div className="rounded-lg border border-border bg-card overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  ...
                  <th className="px-4 py-2.5 text-center text-xs font-medium text-muted-foreground uppercase tracking-wide">Pago</th>
                ...
                {orders.map(order => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      ...
                      {order.email && (
                        <p className="text-[10px] text-muted-foreground/60 mt-0.5">{order.email}</p>
                      )}
                      {!order.facturada && (() => {
                    ...
                    <td className="px-4 py-3 text-center"><FinancialBadge status={order.financial_status} /></td>
                    <td className="px-4 py-3 text-center"><FulfillmentBadge status={order.fulfillment_status} /></td>
        ...
        {(hasPrev || nextCursor) && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-border">
            ...
            <div className="flex items-center gap-2">
              <Button
                size="sm" variant="outline" className="h-7 px-3 gap-1 text-xs"
                onClick={goPrev}
