...
const TYPE_LABEL: Record<string, string> = {
  trending: "Tendencia", classic: "Clásico", gap: "Hueco",
  new_release: "Novedad", adaptation: "Adaptación",
}
...
export default function OportunidadesPage() {
  ...
  const [form, setForm] = useState({
    ...
    category: "", opportunity_type: "trending",
    score: "0", confidence: "medium", notes: "",
  })
  ...
  useEffect(() => { setPage(0); load(0) }, [q, status, type]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (data.ok) { setShowNew(false); load(0) }
    ...
    <div className="p-6 space-y-5 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Select value={type || "all"} onValueChange={v => setType(v === "all" ? "" : v)}>
          <SelectTrigger className="h-8 text-sm w-36"><SelectValue placeholder="Tipo" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="all">Todos</SelectItem>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            ...
            {!loading && rows.length === 0 && (
              <tr><td colSpan={9} className="px-4 py-10 text-center text-muted-foreground">Sin resultados.</td></tr>
            )}
            {rows.map(row => (
              <tr
                key={row.id}
              ...
              >
                ...
                <td className="px-4 py-3">
                  <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${TYPE_COLOR[row.opportunity_type] ?? "bg-muted"}`}>
                ...
                <td className={`px-4 py-3 text-xs font-medium ${CONFIDENCE_COLOR[row.confidence ?? ""] ?? "text-muted-foreground"}`}>
                  {row.confidence ?? "—"}
                </td>
                ...
                <td className="px-4 py-3">
                  {row.in_catalog
                    ? <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400">Sí</span>
                    : <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-muted text-muted-foreground">No</span>
                  }
                </td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="tabular-nums">Pág. {page + 1} de {totalPages}</span>
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex gap-2 flex-wrap">
                <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${TYPE_COLOR[selected.opportunity_type] ?? "bg-muted"}`}>
                ...
                {selected.confidence && (
                  <span className={`text-[10px] px-2 py-0.5 rounded-full bg-muted ${CONFIDENCE_COLOR[selected.confidence] ?? ""}`}>
                    {selected.confidence}
                  </span>
                )}
              </div>
              <div className="grid grid-cols-2 gap-2 text-xs">
                {selected.isbn     && <div><span className="text-muted-foreground">ISBN: </span>{selected.isbn}</div>}
                {selected.author   && <div><span className="text-muted-foreground">Autor: </span>{selected.author}</div>}
                {selected.publisher && <div><span className="text-muted-foreground">Editorial: </span>{selected.publisher}</div>}
                {selected.category && <div><span className="text-muted-foreground">Categoría: </span>{selected.category}</div>}
                <div><span className="text-muted-foreground">Score: </span><strong>{Number(selected.score).toFixed(2)}</strong></div>
                {selected.ml_price_avg && <div><span className="text-muted-foreground">Precio ML: </span>${Number(selected.ml_price_avg).toLocaleString("es-AR")}</div>}
                {selected.ml_listings_count != null && <div><span className="text-muted-foreground">Listings ML: </span>{selected.ml_listings_count}</div>}
                {selected.ml_sales_rank && <div><span className="text-muted-foreground">Rank ML: </span>#{selected.ml_sales_rank}</div>}
                <div><span className="text-muted-foreground">En catálogo: </span>{selected.in_catalog ? "Sí" : "No"}</div>
              </div>
              {selected.notes && (
              ...
              {selected.tags && selected.tags.length > 0 && (
                <div className="flex gap-1.5 flex-wrap">
                  {selected.tags.map(t => <span key={t} className="text-[10px] bg-muted px-2 py-0.5 rounded-full">{t}</span>)}
                </div>
              )}
            </div>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                ...
                <Input className="h-8 text-sm" value={form.isbn} onChange={e => setForm(f => ({ ...f, isbn: e.target.value }))} />
              </div>
              <div className="space-y-1">
                <Label className="text-xs">Tipo *</Label>
                <Select value={form.opportunity_type} onValueChange={v => setForm(f => ({ ...f, opportunity_type: v }))}>
              ...
              <div className="space-y-1">
                ...
                <Input className="h-8 text-sm" value={form.author} onChange={e => setForm(f => ({ ...f, author: e.target.value }))} />
              </div>
              <div className="space-y-1">
                <Label className="text-xs">Score (0-100)</Label>
                <Input className="h-8 text-sm" type="number" min={0} max={100} value={form.score} onChange={e => setForm(f => ({ ...f, score: e.target.value }))} />
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1">
                ...
                <Input className="h-8 text-sm" value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} />
              </div>
              <div className="col-span-2 space-y-1">
                <Label className="text-xs">Notas</Label>
                <Textarea className="text-sm min-h-[80px]" value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} />
              </div>
            </div>
          </div>
          <DialogFooter>
