...
type CronFailure = { error: true; response: NextResponse; user: null; via: null }
type CronResult = CronSuccess | CronFailure

export async function protectCron(request: NextRequest): Promise<CronResult> {
  const cronSecret = process.env.CRON_SECRET
  const headerSecret = request.headers.get("x-cron-secret")

  // 1. Allow cron / service-to-service calls via shared secret
  if (cronSecret && headerSecret && headerSecret === cronSecret) {
    return { error: false, response: null, user: null, via: "cron" }
  }
  ...
  try {
    ...
    if (!error && user) {
      return { error: false, response: null, user, via: "session" }
    }
  } catch {
    /* ignore */
  }
...
type AuthSuccess = { error: false; response: null; user: import("@supabase/supabase-js").User }
type AuthFailure = { error: true; response: NextResponse; user: null }
type AuthResult = AuthSuccess | AuthFailure

export async function protectAPI(): Promise<AuthResult> {
  try {
    const supabase = await createClient()
    const {
      data: { user },
    ...
    if (error || !user) {
      return {
        error: true,
        response: NextResponse.json({ error: "unauthorized", message: "Autenticación requerida" }, { status: 401 }),
        user: null,
      }
    }
    ...
    return {
      error: true,
      response: NextResponse.json({ error: "auth_error", message: "Error verificando autenticación" }, { status: 500 }),
      user: null,
    }
  }
}
