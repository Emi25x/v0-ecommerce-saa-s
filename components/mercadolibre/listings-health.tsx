...
const HEALTH_OPTS = [
  {
    value: "soon_to_be_paused",
    label: "Próximas a pausarse",
    color: "text-red-400",
    bg: "bg-red-500/10 border-red-500/20",
  },
  {
    value: "catalog_not_listed",
    ...
    <div className="space-y-4">
        ...
        {HEALTH_OPTS.map((opt) => (
          ...
          >
            ...
            {healthFilter === opt.value && total > 0 && (
              <span className={`ml-1 text-xs rounded-full px-1.5 py-0.5 bg-current/10`}>{total}</span>
            )}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-border overflow-hidden">
          {/* Banner explicativo */}
          <div
            className={`px-4 py-2.5 text-xs flex items-center gap-2 ${activeOpt.bg} ${activeOpt.color} border-b border-current/10`}
          >
            {healthFilter === "soon_to_be_paused" ? (
