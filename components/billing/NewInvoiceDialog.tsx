...
import {
  Plus, RefreshCw, Receipt, CheckCircle2, XCircle, Trash2,
  Barcode, Loader2, X, ShieldCheck,
} from "lucide-react"
...
interface NewInvoiceDialogProps {
  ...
  setSkuStatus: (fn: ("idle"|"loading"|"found"|"notfound")[] | ((prev: ("idle"|"loading"|"found"|"notfound")[]) => ("idle"|"loading"|"found"|"notfound")[])) => void
...
export function NewInvoiceDialog({
  open, onOpenChange,
  newForm, setNewForm,
  items, addItem, removeItem, updateItem, totales,
  skuInput, setSkuInput, skuStatus, setSkuStatus, lookupProduct,
  padronStatus, padronMsg, lookupPadron, setPadronStatus, setPadronMsg,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Receipt className="h-5 w-5" />Nueva factura electr&oacute;nica</DialogTitle>
        </DialogHeader>
          ...
          <div className="rounded-lg border border-border p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2 space-y-1.5">
                ...
                <Input placeholder="Juan Garc&iacute;a" value={newForm.receptor_nombre} onChange={e => setNewForm(p => ({ ...p, receptor_nombre: e.target.value }))} />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                <Label>
                  ...
                  {newForm.tipo_doc_receptor === "99"
                    ...
                    : <span className="text-muted-foreground font-normal ml-1 text-xs">{'\u2014'} Enter para buscar en padr&oacute;n ARCA</span>
                  }
                </Label>
          ...
          <div className="rounded-lg border border-border p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <Button variant="outline" size="sm" onClick={addItem} className="gap-1 h-7">
                <Plus className="h-3 w-3" />Agregar &iacute;tem
              </Button>
            </div>
                  ...
                  <div key={idx} className="space-y-1">
                    ...
                    {status === "notfound" && (skuInput[idx] ?? "").trim() && (
                      <p className="text-xs text-amber-400 pl-[118px]">
                        No se encontr&oacute; &quot;{skuInput[idx]}&quot; en la base de productos. Pod&eacute;s escribir la descripci&oacute;n manualmente.
                      </p>
                    )}
                  </div>
                )
              ...
              return (
                <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 p-3 text-sm text-amber-400 flex items-start gap-2">
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p>El total supera <strong>$10.000.000</strong>. Es obligatorio identificar al receptor con CUIT, CUIL, CDI o DNI. Seleccion&aacute; el tipo de documento e ingres&aacute; el n&uacute;mero.</p>
                  </div>
                </div>
              )
            }
            if (total >= 208_644 && total < 10_000_000 && docTipo === "99") {
              return (
                <div className="rounded-lg border border-blue-500/30 bg-blue-500/10 p-3 text-sm text-blue-400 flex items-start gap-2">
                  <ShieldCheck className="h-4 w-4 flex-shrink-0 mt-0.5" />
                  <p>Pod&eacute;s emitir sin identificar al receptor hasta <strong>$10.000.000</strong> (RG 5700/2025). El monto actual es ${total.toLocaleString("es-AR")}.</p>
                </div>
              )
            }
            return null
