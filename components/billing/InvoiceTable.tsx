...
import { FileText, RefreshCw, Download, Search, ChevronLeft, ChevronRight, Loader2, Building2 } from "lucide-react"
import { TIPO_COMPROBANTE, fmtFecha, fmtMoney, nroFmt } from "./types"
import type { ArcaConfig, Factura } from "./types"

function estadoBadge(estado: string) {
  if (estado === "emitida")
    return <Badge className="bg-emerald-500/15 text-emerald-400 border-emerald-500/30 text-xs">Emitida</Badge>
  if (estado === "pendiente")
      ...
      {!config && (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4 mb-4 flex items-center gap-3">
          ...
          <p className="text-sm text-amber-300">
            ...
            <button onClick={() => setActiveTab("config")} className="underline font-semibold">
              Configuracion ARCA
            </button>
            .
          </p>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              ...
              <th className="p-3 text-right text-xs font-medium text-muted-foreground uppercase hidden md:table-cell">
                Total
              </th>
              <th className="p-3 text-left text-xs font-medium text-muted-foreground uppercase">Estado</th>
              <th className="p-3 text-left text-xs font-medium text-muted-foreground uppercase hidden lg:table-cell">
                CAE
              </th>
